...
const ACTS = [
  { id: "scan", label: "Scan the address", sub: "Aerial + county parcel", ms: 5600, Icon: MapPin },
  { id: "draw", label: "Drive the fence", sub: "Backyard, tie-ins, gates", ms: 8600, Icon: Ruler },
  { id: "design", label: "See it built", sub: "True-to-spec 3D", ms: 7600, Icon: Boxes },
  { id: "price", label: "Build the proposal", sub: "Good / Better / Best", ms: 7400, Icon: FileText },
  { id: "send", label: "Send to the client", sub: "Opened. Signed. Paid.", ms: 8200, Icon: Send },
] as const;

const STATUS = ["Scanning", "Measuring", "Designing", "Pricing", "Sent"] as const;
...
const CUM: number[] = (() => {
  ...
  for (let i = 1; i < FENCE_RUN.length; i++) {
    c.push(c[i - 1] + Math.hypot(FENCE_RUN[i].x - FENCE_RUN[i - 1].x, FENCE_RUN[i].y - FENCE_RUN[i - 1].y));
  }
  return c;
})();
...
function Callout({
  ...
  const fill = tone === "accent" ? "#16A34A" : tone === "pink" ? GATE_PINK : tone === "dark" ? "#0B1410" : "#FFFFFF";
  ...
  return (
    ...
    >
      <rect width={w} height={22} rx={11} fill={fill} opacity={tone === "light" ? 0.96 : 0.94} />
      <text
        x={w / 2}
        y={15}
        textAnchor="middle"
...
function PanelHead({ icon: Icon, title }: { icon: typeof MapPin; title: string }) {
  return (
...
export function DemoWalkthrough({
  /** Open on a given act instead of the top — the /demo/wt preview route
   *  uses it to inspect a single beat. */
  initialAct = 0,
}: {
  initialAct?: number;
}) {
  ...
  const [act, setAct] = useState(Math.min(Math.max(0, initialAct), ACTS.length - 1));
  ...
  const engine = useMemo(() => {
    ...
    const tiers = fenceTiers(FENCE_TYPE_ID).map((t) => ({
      ...
      price: priceFence({ ...layout, type: t.type, stain: t.stain }, { markupPct: t.markupPct }),
    }));
    const bomBy = (k: string) => takeoff.bom.find((b) => b.key === k)?.qty ?? 0;
    const type = fenceType(FENCE_TYPE_ID);
    const pickets =
      bomBy("pickets") || takeoff.bom.find((b) => /picket|board/i.test(b.label))?.qty || 0;
    const concrete = takeoff.bom.find((b) => /concrete/i.test(b.label))?.qty ?? 0;
    ...
    () =>
      seen3d ? (
        <Fence3D
          ...
          gates={GATES.map((g) => ({ x: g.x, y: g.y, kind: g.kind, widthFt: g.widthFt }))}
          heightFt={FENCE_HEIGHT_FT}
        ...
        <div className="relative overflow-hidden rounded-[22px] bg-[#0B1410] shadow-[0_40px_90px_-40px_rgba(10,43,25,0.6)] ring-1 ring-black/5">
          ...
          <div className="flex items-center gap-2.5 border-b border-white/10 bg-[#0d1a12] px-3.5 py-2.5">
            ...
            <p className="min-w-0 flex-1 truncate text-[12.5px] font-medium text-white">
              {DEMO_ADDRESS}
              <span className="ml-1.5 font-normal text-white/40">{DEMO_CITY}</span>
            </p>
            <span
              className={cn(
            ...
            >
              {STATUS[act]}
              {act < 4 && !reduceMotion ? <span className="anim-ellipsis">…</span> : null}
            </span>
          </div>
          ...
          <div className="relative aspect-[900/580] w-full">
            ...
            >
              <svg viewBox="0 0 900 580" className="block h-full w-full" role="img" aria-label="Demo property scan: aerial view with the county parcel boundary and the backyard fence line traced">
                {aerial}
                  ...
                  <g opacity={1 - beat(actT, 0.3, 0.42)}>
                    <defs>
                      <linearGradient id="dw-sweep" x1="0" y1="0" x2="0" y2="1">
                        ...
                        <stop offset="72%" stopColor={TRACE} stopOpacity="0.28" />
                        <stop offset="100%" stopColor={TRACE} stopOpacity="0.95" />
                      </linearGradient>
                    </defs>
                    <rect
                      x={0}
                ...
                {(() => {
                  ...
                  return (
                    <g>
                      ...
                      />
                      {PARCEL.map((pt, i) => {
                        const s = act === 0 ? beat(actT, 0.5 + i * 0.045, 0.6 + i * 0.045) : 1;
                        return s <= 0 ? null : (
                          <circle
                            key={i}
                            cx={pt.x}
                ...
                {act === 0 && (
                  <>
                    <Callout x={450} y={62} text={`Parcel ${DEMO_APN}`} show={beat(actT, 0.6, 0.72)} />
                    <Callout
                      x={200}
                      y={534}
                ...
                {act === 1 && (
                  <g>
                    ...
                    <path d={fenceD} fill="none" stroke={CASING} strokeOpacity={0.6} strokeWidth={9} strokeLinejoin="round" strokeLinecap="round" />
                    <path d={fenceD} fill="none" stroke={TRACE} strokeWidth={3.6} strokeLinejoin="round" strokeLinecap="round" />

                    {/* pen head */}
                    {drawT > 0 && drawT < 1 && (
                      <g>
                        <circle cx={head.x} cy={head.y} r={13} fill={TRACE} opacity={0.22} />
                        <circle cx={head.x} cy={head.y} r={5.5} fill="#fff" stroke="#16A34A" strokeWidth={2.5} />
                      </g>
                    )}
                    ...
                    {/* house tie-ins */}
                    {[FENCE_RUN[0], FENCE_RUN[FENCE_RUN.length - 1]].map((p, i) => {
                      const s = i === 0 ? clamp01(penPx / 14) : clamp01((penPx - RUN_PX + 14) / 14);
                      return s <= 0 ? null : (
                        <g key={i} opacity={s}>
                          <circle cx={p.x} cy={p.y} r={9} fill="none" stroke={TRACE} strokeWidth={2} opacity={0.5} />
                          <circle cx={p.x} cy={p.y} r={4.5} fill={TRACE} stroke="#0B1410" strokeWidth={1.6} />
                        </g>
                      );
                    })}
                      ...
                      return (
                        <g key={i} opacity={s}>
                          ...
                          <circle
                            ...
                            opacity={(1 - s) * 0.9}
                          />
                          <Callout x={g.x} y={g.y - 26} text={g.label} show={s} tone="pink" />
                        </g>
                      );
                    })}
            ...
            <div
              ...
              style={{ opacity: showPaper ? 1 : 0, pointerEvents: showPaper ? undefined : "none" }}
            >
              {showPaper && (
                <ProposalStage
                  act={act}
            ...
            >
              {act === 0 && (
                <>
                  ...
                  <div className="divide-y divide-white/[0.07]">
                    ...
                    <Stat k="Lot" v={`${LOT_ACRES} ac`} hint={`${LOT_SQFT.toLocaleString("en-US")} sq ft`} />
                    <Stat k="House footprint" v={`${HOUSE_SQFT.toLocaleString("en-US")} sq ft`} />
              ...
              {act === 1 && (
                <>
                  ...
                  <div className="divide-y divide-white/[0.07]">
                    ...
                    <Stat k="Net fabric" v={`${engine.takeoff.netFenceLf} LF`} hint="gates out" />
                    <Stat k="Corners" v={String(CORNERS)} />
                    <Stat k="House tie-ins" v={String(ENDS)} />
                    <Stat k="Gates" v={`${GATES.length}`} hint="4′ walk + 10′ drive" />
                  </div>
              ...
              {act === 2 && (
                <>
                  ...
                  <div className="divide-y divide-white/[0.07]">
                    <Stat k="Fence" v={engine.type.label} hint={`${FENCE_HEIGHT_FT}′`} />
                    <Stat k="Sections" v={String(engine.takeoff.sections)} hint={`${engine.type.postSpacingFt}′ o.c.`} />
                    <Stat k="Posts" v={String(engine.takeoff.posts.total)} hint={`${engine.takeoff.posts.corner} corner`} />
                    <Stat k="Rails" v={String(engine.rails)} />
                    <Stat k="Pickets" v={engine.pickets.toLocaleString("en-US")} />
                    <Stat k="Concrete" v={`${engine.concrete} bags`} />
                    <Stat k="Crew hours" v={String(engine.takeoff.laborHours)} />
                  </div>
                  <p className="mt-4 rounded-lg bg-accent-500/10 px-3 py-2.5 text-[12.5px] leading-relaxed text-accent-200">
                    {engine.type.spec.infillMaterial} on {engine.type.spec.railMaterial},
                    set on {engine.type.spec.postMaterial}.
                  </p>
                </>
              )}
              ...
              {act === 3 && (
                <>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="mt-0.5 flex items-baseline justify-between">
                            <p className="text-[11.5px] text-white/45">{t.spec.label}</p>
                  ...
                  <p className="mt-3 rounded-lg bg-white/[0.05] px-3 py-2.5 text-[12px] leading-relaxed text-white/55">
                    ...
                    same {engine.takeoff.netFenceLf} LF the 3D model was built from.
                  </p>
                </>
              )}
              ...
              {act === 4 && (
                <>
                  ...
                  <div className="mt-3 space-y-3">
                    {[
                      { t: "9:41 AM", l: "Proposal sent", s: "whitakers@example.com", at: 0.16, Icon: Mail },
                      { t: "9:53 AM", l: "Opened on mobile", s: "Viewed the 3D model twice", at: 0.4, Icon: Sparkles },
                      { t: "10:06 AM", l: "Signed", s: `Better · ${money(engine.chosen.price.total)}`, at: 0.62, Icon: CheckCircle2 },
                      { t: "10:07 AM", l: "Deposit paid", s: `${money(deposit)} · 30%`, at: 0.8, Icon: Check },
                    ].map((r, i, arr) => {
                      const on = actT > r.at;
                      return (
                        <div key={r.l} className="relative flex gap-3">
                          ...
                          <span
                            className={cn(
                              ...
                              on ? "bg-accent-500 text-white" : "bg-white/10 text-white/30",
                            )}
                          >
                          ...
                          <div
                            className={cn(
                              ...
                              on ? "translate-x-0 opacity-100" : "-translate-x-1 opacity-30",
                            )}
                          >
                            ...
                            <p className="truncate text-[11.5px] text-white/45">{r.s}</p>
                          </div>
                        </div>
                      );
                    })}
                  ...
                  <div
                    className={cn(
                      ...
                      actT > 0.86 ? "scale-100 opacity-100" : "scale-95 opacity-0",
                    )}
                  >
        ...
        {ACTS.map((a, i) => {
          ...
          return (
            ...
            >
              ...
              <p className="mt-1 truncate pl-7 text-[11.5px] text-zinc-400">{a.sub}</p>
              <span className="absolute inset-x-0 bottom-0 h-[3px] bg-zinc-100">
                <span
                  className="block h-full bg-accent-500"
                  style={{ width: `${(done && !active ? 1 : active ? actT : 0) * 100}%` }}
                />
              </span>
            </button>
          );
      ...
      {!reduceMotion && (
        <div className="mt-3 flex items-center justify-center gap-2">
          ...
          <p className="text-[11.5px] text-zinc-400">
            Prebuilt demo property — every figure computed live by the FenceScan engine.
          </p>
        </div>
      )}
    </div>
...
function ProposalStage({
  ...
  useEffect(() => {
    ...
    if (!el) return;
    const measure = () => setScale(el.clientWidth / 900);
    measure();
  ...
  const sending = act === 4;
  const shrink = sending ? (reduceMotion ? 1 : easeOut(beat(t, 0, 0.24))) : 0;
  const pageScale = 1 - shrink * 0.42;
  ...
  return (
    <div ref={boxRef} className="h-full w-full overflow-hidden bg-[#0e1a12]">
      ...
      >
        ...
        >
          {/* ---- package cards ---- */}
          <div className="absolute flex" style={{ left: 18, top: 90, width: 812, gap: 14 }}>
            {tiers.map((tier, i) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="mt-1.5 text-[10px] leading-none tabular-nums text-zinc-400">
                    {money(tier.price.pricePerLf)}/LF · {tier.warrantyYears}-yr warranty
                  </p>
            ...
            <div className="mt-2.5 grid grid-cols-1 gap-y-[7px]">
              {scope.map((line) => (
                <div key={line} className="flex items-start gap-2">
                  ...
                  <p className="text-[11px] leading-[1.35] text-zinc-600">{line}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          >
            <svg viewBox="170 58 576 470" className="block h-full w-full">
              <polygon points={ptsAttr(PARCEL)} fill="#fff" stroke="#BFDEC7" strokeWidth={4} strokeDasharray="14 11" />
              <polygon points={ptsAttr(HOUSE)} fill="#E4E0D6" stroke="#B9B2A3" strokeWidth={4} />
              <path
              ...
              />
              {FENCE_RUN.slice(1, -1).map((p, i) => (
                <rect key={i} x={p.x - 7} y={p.y - 7} width={14} height={14} fill="#fff" stroke="#0D1B12" strokeWidth={4} />
              ))}
              {GATES.map((g, i) => {
            ...
            <p className="absolute bottom-1.5 left-2.5 rounded-full bg-white/90 px-2 py-[3px] text-[9px] font-semibold text-zinc-600 shadow-sm">
              Backyard enclosure · {SEGMENT_FT.length} runs · {GATES.length} gates
            </p>
          </div>
          ...
          <div
            ...
            style={{ left: 18, top: 488, width: 812, opacity: enter(0.7, 0.86) }}
          >
            <p className="text-[10px] text-zinc-400">
              Prepared from a FenceScan takeoff · pricing valid 30 days · permit
              fees billed at cost
            </p>
        ...
        {sending && (
          <div
            ...
            style={{
              ...
              top: 290,
              width: 322,
              transform: `translateY(-50%) translateX(${(1 - enterSend(t, reduceMotion)) * 26}px)`,
          ...
          >
            <div className="overflow-hidden rounded-xl bg-white shadow-[0_30px_70px_-18px_rgba(0,0,0,0.8)]">
              <div className="flex items-center gap-2 border-b border-zinc-100 px-3.5 py-2.5">
                ...
                <p className="text-[12px] font-semibold text-zinc-900">Send proposal</p>
              </div>
              <div className="space-y-2 px-3.5 py-3 text-[11px]">
                {[
                  ...
                  <div key={k} className="flex gap-2.5">
                    <span className="w-[52px] shrink-0 text-zinc-400">{k}</span>
                    <span className="truncate font-medium text-zinc-800">{v}</span>
                  </div>
                ))}
              </div>
              <div className="px-3.5 pb-3.5">
                <span
                  className={cn(
                    ...
                    t > 0.3 ? "scale-[0.97] bg-accent-700" : "scale-100 bg-accent-600",
                  )}
                >
                  {t > 0.38 || reduceMotion ? (
                    <>
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[11px] leading-tight text-white/75">
                    {money(chosen.price.total)} · {money(deposit)} deposit paid
                  </p>
                </div>
              </div>
            </div>
